...
if __name__ == '__main__':
    ...
    print('TESTING OMEGA CN7500 MODBUS MODULE')
    ...
    #instr = minimalmodbus.Instrument('/dev/ttyUSB0', 1, minimalmodbus.MODE_ASCII, debug=True)
    instr.serial.port = '/dev/ttyUSB0'  # this is the serial port name
    instr.serial.baudrate = 9600  # Baud
    instr.serial.bytesize = 8
    instr.serial.parity = serial.PARITY_NONE
    instr.serial.stopbits = 1
    instr.serial.timeout = 0.5  # seconds

    instr.address = 1  # this is the slave address number
    instr.mode = minimalmodbus.MODE_ASCII  # rtu or ascii mode
    instr.clear_buffers_before_each_transaction = True

    print(instr)

    
    print('Control:                {0}'.format(instr.get_control_mode()))
    print('SP:                     {0}'.format(instr.get_setpoint()))
    print('PV:                     {0}'.format(instr.get_pv()))
    print('OP1:                    {0}'.format(instr.get_output1()))
    print('Is running:             {0}'.format(instr.is_running()))
    print('Start pattern:          {0}'.format(instr.get_start_pattern_no()))
